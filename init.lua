...
require("lazy").setup({
	...
	{ -- Fuzzy Finder (files, lsp, etc)
		...
		config = function()
			...
			vim.keymap.set("n", "<leader>sp", function()
				builtin.find_files({ cwd = vim.fn.expand("~/IdeaProjects/") })
			end, { desc = "[S]earch [P]rojects" })
			vim.keymap.set("n", "<leader>sa", function()
				builtin.find_files({ cwd = vim.fn.expand("~/IdeaProjects/dijkstra-map/") })
			end, { desc = "[S]earch [A]ssignment" })

			vim.keymap.set("n", "<leader>rj", function()
				local root = vim.fs.root(0, { "pom.xml" })
				if not root then
					return vim.notify("No pom.xml found", vim.log.levels.ERROR)
				end
				vim.cmd("split | resize 15 | terminal cd " .. vim.fn.shellescape(root) .. " && mvn compile exec:java")
			end, { desc = "[R]un [J]ava" })

			vim.keymap.set("n", "<leader>rc", function()
				local root = vim.fs.root(0, { "CMakeLists.txt" })
				if not root then
					return vim.notify("No CMakeLists.txt found", vim.log.levels.ERROR)
				end
				vim.cmd(
					"split | resize 15 | terminal cd "
						.. vim.fn.shellescape(root)
						.. " && cmake -S . -B build -G Ninja && cmake --build build && build\\eclipse.exe"
				)
			end, { desc = "[R]un [C]++" })
		end,
	},
	...
	{ -- Autoformat
		...
		opts = {
			...
			-- clang-format auto-discovers a .clang-format file in the project root.
			formatters = {
				["clang-format"] = {
					-- Use LLVM's real .exe, not the Mason .CMD shim: the batch shim
					-- routes through cmd.exe and mangles args that contain a space
					-- (our home path has one: "SR Gears").
